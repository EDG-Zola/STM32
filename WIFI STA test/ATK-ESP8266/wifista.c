...
PRESTA:
	...
	//			Show_Str(30,50,200,16,"正在配置ATK-ESP模块,请稍等...",12,0);
				...
				sprintf((char*)p,"AT+CIPSTART=\"TCP\",\"%s\",%s",ipbuf,(u8*)portnum);    //配置目标UDP服务器
				delay_ms(200);
...
//			Show_Str(30,50,200,16,"正在配置ATK-ESP模块,请稍等...",12,0);
			...
			delay_ms(1000);         //延时3S等待返回数据OK
			delay_ms(1000);
			delay_ms(1000);
			delay_ms(1000);
			//第五步，建立TCP Client到ip地址"xx.xxx.xx.xxx"
			sprintf((char*)p,"AT+CIPSTART=\"TCP\",\"%s\",%d",ipbuf,8080);    //配置目标TCP服务器
			//***********************************************这里的p的值没有加上端口号，在调试的时候加上端口号后能进行发送数据**********/
			while(atk_8266_send_cmd(p,"OK",200))	//0表示发送成功，1表示发送失败
			{
//					LCD_Clear(WHITE);
//					POINT_COLOR=RED;
...
//					if(key==WKUP_PRES)//退出STA模式
						goto PRESTA;
			}
			//第六步，开启透传模式			
			atk_8266_send_cmd("AT+CIPMODE=1","OK",200);      //传输模式为：透传	
			
			delay_ms(1000);//等待3s，启动透传模式	
			delay_ms(1000);
			delay_ms(1000);
			delay_ms(1000);
		}
...
//			Show_Str(120+30,80,200,12,(u8*)ATK_ESP8266_WORKMODE_TBL[netpro],12,0); 		//连接状态
			...
			{
//				key=1;//表示KEY0按下，发送数据
//				netpro=1;//表示选择TCP 客户端模式
				timex=100;
				atk_8266_quit_trans();
				//第七步，开始传输数据
				atk_8266_send_cmd("AT+CIPSEND","OK",20);         //开始透传           
				sprintf((char*)p,"%sid:4\t测试数据:%d\r\n",ATK_ESP8266_WORKMODE_TBL[netpro],++t);//测试数据
//				Show_Str(30+54,100,200,12,p,12,0);
				//***********************发送数据给串口
				u3_printf("%s",p);
				/******************************************直接选择TCP Client模式，并发送数据************/
//				key=KEY_Scan(0);
...
//					if(constate!='+')t=1000;		//状态为还未连接,立即更新连接状态
				...
				if(t==10000)//连续10秒钟没有收到任何数据,检查连接是不是还存在.
				{
