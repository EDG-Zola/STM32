...
//				if(iplen<15)
//				{ 
//					ip[iplen++]=kbd_tbl[key-1][0];//把按下的值（ip地址）赋值给ip数组
//				}
//			}else
//			{
//				if(key==13)if(iplen)iplen--;	//按下了删除键  
//				if(key==14&&iplen)break; 		//按下确认连接键 
//				if(key==15){res=1;break;}		//按下取消连接键
//			} 
//			ip[iplen]=0; 
...
//	u16 rlen=0;
	...
	while(atk_8266_send_cmd("AT","OK",20))//检查WIFI模块是否在线,函数返回1时，表示未连接成功
	{
	...
	while(1)
	{
		...
		/****************************直接选择WIFI-STA 测试模式，即STA客户端模式*******/
//		key=KEY_Scan(0); //检测按键值，按下key0，key=1,按下key1,key=2
//		if(key)
//		{
//			LCD_Clear(WHITE);
//			POINT_COLOR=RED;
